...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, K: int) -> int:
        ...
        Q = [(0, src, layover)]
        while Q:
            price, city_arrive, layover = heapq.heappop(Q)
            if city_arrive == dst:
                return price
            if layover <= K:
                layover += 1
                for city_arrive_next, price_next in graph[city_arrive]:
                    ...
                    heapq.heappush(Q, (price_total, city_arrive_next, layover))
                    print('from ', city_arrive, ' to ', city_arrive_next, ' with ', price_total)
        return -1
